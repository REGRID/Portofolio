  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (canvasRef.current && typeof window !== 'undefined') {
        ...
        // When 1 card is centered (minDist <= 55px): otherCardAllowance = 0.0 -> ONLY 1 card is colored (NO radial mask)!
        // When panning/sliding between cards: otherCardAllowance opens smoothly so incoming card reveals its color
      ...
      const tl = gsap.timeline({
        onComplete: () => {
          ...
          cachedTilesRef.current.forEach((tile) => {
            ...
            tile.lastOpacity = dist < centerDist ? 1 : 0;
            const normX = (cardCenterX - scX) / scX;
            const normY = (cardCenterY - scY) / scY;
            const px = Math.max(-28, Math.min(28, -normX * 18));
